  ...
  const artefacts = [
    {
      file: 'spec.md',
      sub: f.specSub,
      excerpt: ['# Problem', 'First paint flashes', 'wrong theme.', '', '# AC-1', 'Paint matches pref.'],
    },
    {
      file: 'plan.md',
      sub: f.planSub,
      excerpt: ['# Stack', 'Next.js · CSS vars', 'next-themes 0.4', '', '# Decisions', 'SSR-safe cookie.'],
    },
    {
      file: 'tasks.md',
      sub: f.tasksSub,
      excerpt: ['[x] T-01 install', '[x] T-02 useTheme', '[ ] T-03 Playwright', '    verify AC-1', '[ ] T-04 Lighthouse', '    verify AC-3'],
    },
  ]
    ...
    <section className="skl-flow">
      <div className="mx-auto max-w-6xl px-6 lg:px-8 py-24 sm:py-32 border-t border-border/60">
        <div className="max-w-2xl mb-14">
          ...
          <h2 className="skl-ed-h2">{f.heading}</h2>
          <p className="mt-6 text-[15.5px] leading-[1.65] text-muted-foreground">{f.intro}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-border/60 rounded-lg overflow-hidden border border-border/60">
          {artefacts.map((a, i) => (
            <div key={a.file} className="skl-flow__col bg-background p-7">
              ...
              <p className="text-[14px] text-muted-foreground leading-[1.55] mb-6">{a.sub}</p>
